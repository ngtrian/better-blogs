...
class XfAddOns_Blogs_ViewPublic_Blog_Rss extends XenForo_ViewPublic_Base
{
	...
	public function renderRss()
	{
		...
		$feed->setTitle($blog['blog_title']);
		$feed->setDescription(!empty($blog['description']) ? $blog['description'] : $blog['blog_title']);
		$feed->setLink(XenForo_Link::buildPublicLink('canonical:xfa-blogs', $blog), 'rss');
		if (!$buggyXmlNamespace)
		{
			$feed->setFeedLink(XenForo_Link::buildPublicLink('canonical:xfa-blogs/-/index.rss', $blog), 'rss');
		}
		if ($lastUpdate > 0)
		{
