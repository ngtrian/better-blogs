 ...
 */
class XfAddOns_Blogs_DataWriter_BlogWatch extends XenForo_DataWriter
{
	...
	{
		$fields = array();
		$fields['xfa_blog_watch']['watch_id'] = array(
			'type' => self::TYPE_UINT,
			'autoIncrement' => true
		);
		$fields['xfa_blog_watch']['user_id'] = array(
			'type' => self::TYPE_UINT,
			'required' => true
		);
		$fields['xfa_blog_watch']['blog_user_id'] = array(
			'type' => self::TYPE_UINT,
			'required' => true
		);
		return $fields;
	...
	protected function _getExistingData($key)
	{
		if ($key)
		{
			...
			$data = $db->fetchRow("SELECT * FROM xfa_blog_watch WHERE watch_id = ?", array( $key ));
			return array( 'xfa_blog_watch' => $data );
		}
		return null;
	}
